...
class ReplyMapperTest {
    ...
    @Test
    @DisplayName("77번 게시물의 댓글 목록을 조회했을 때 결과리스트의 사이즈는12이어야 한다")
    void  findAllTest() {
        //given
        long boardNo = 77L;
        //when
        ...
        //then
        assertEquals(12,replyList.size());
        assertEquals("잼민이 225", replyList.get(0).getReplyWriter());
    }

    @Test
    @DisplayName("77번 게시물의 306번 댓글을 삭제하면 306번 댓글은 조회되지 않을 것이고" + "77번을 전체조회하면 리스트의 사이즈는 11이어야함")
    void deleteTest() {
        //given
        long boardNo = 77;
        long replyNo = 306;
        ...
        //then
        assertNull(reply);
        assertEquals(11, replyMapper.findAll(boardNo).size());
    }

    @Test
    @DisplayName("225번 댓글의 댓글 내용을 수정하면 다시 조회했을 때 수정된 내용이 조회")
    void modifyTest() {
        //given
        long replyNo = 225;
